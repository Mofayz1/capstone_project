...
function sendSms($user, $type, $shortCodes = [])
{
    ...
    if ($general->sn == 1 && $smsTemplate) {
        ...
        $message = shortCodeReplacer("{{name}}", $user->name, $message);
        $sendSms->$gateway($user->phone,$general->sitename,$message,$general->sms_config);
    }
}
...
function sendEmail($user, $type = null, $shortCodes = [])
{
    ...
    if ($config->name == 'php') {
        sendPhpMail($user->email, $user->name,$emailTemplate->subj, $message, $general);
    } else if ($config->name == 'smtp') {
        sendSmtpMail($config, $user->email, $user->name, $emailTemplate->subj, $message,$general);
    } else if ($config->name == 'sendgrid') {
        sendSendGridMail($config, $user->email, $user->name, $emailTemplate->subj, $message,$general);
    } else if ($config->name == 'mailjet') {
        sendMailjetMail($config, $user->email, $user->name, $emailTemplate->subj, $message,$general);
    }
}
...
function advertisements($size) {
    ...
    if ($ad) {
        if ($ad->type == 1) {
            ...
            return  '<a  target="_blank" href="'.route('add.clicked',encrypt($ad->id)).'" class="my-3"><img src="'.getImage('assets/images/advertisement/'.$ad->image).'" alt="image"></a>';            
        }
        if($ad->type == 2) {
            impressionCount($ad->id);
